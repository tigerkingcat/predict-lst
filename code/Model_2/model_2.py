...
# best_features = None
#
# # Forward selection: Iterate through all possible combinations
...
#
#         # Fit model and calculate R² score
...
# Update model to use Polynomial Features
...
for fold, (train_index, test_index) in enumerate(tscv.split(X)):
    ...
    print(f"Fold {fold + 1}")
    print("X_test NaN values count:", np.isnan(X_test).sum())
    # Fit the model
...
# Bar plot of Coefficients
coeff_mean = np.mean(np.array(coefficients), axis=0)
print(coefficients)
