...
# Load the data
df = pd.read_csv('LA_LST_Impervious_Surface_1km.csv')
print(df.info())

# Drop unnecessary column
df.drop(axis=1, columns='.geo', inplace=True)
df = df[df['Impervious_1km'] > 1]

# Correlation analysis
print(df.corr())

# Scatterplot of Impervious Surface vs. Land Surface Temperature
sns.scatterplot(data=df, x='Impervious_1km', y='LST_Celsius')
...
sns.scatterplot(data=df, x='Impervious_1km', y='LST_Celsius', label='Actual Data')

# Generate the regression line points
x_range = np.linspace(df['Impervious_1km'].min(), df['Impervious_1km'].max(), 100)
