...
combined_df = pd.read_csv('../../data/step2-aggregate/Lst_impervious_combined.csv')
...
combined_df = combined_df.rename(columns={
    'latitude_impervious': 'latitude',
    'longitude_impervious': 'longitude',
    'year_impervious': 'year'
})
gdf_combined = to_geodataframe(combined_df)
gdf_pv = to_geodataframe(pv_df)


# Perform KDTree nearest neighbor search by year for LST and Impervious DataFrames
merged_gdf = kdtree_nearest_by_year(gdf_combined, gdf_pv, 'lst', 'pv')
...
merged_df.to_csv('../../data/step2-aggregate/pv_lst_imp.csv', index=False)